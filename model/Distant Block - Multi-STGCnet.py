...
for i in range(begin*interval_batch,intervals):
    ...
    # lstm的样本
    sample_distant = out_standard.values[i-pre_sr:i]
    samples_distant.append(sample_distant)
    labels.append(label)

num_samples = len(labels)
samples_distant = np.reshape(np.array(samples_distant),(num_samples,118,pre_sr))
labels = np.array(labels)
print(samples_distant.shape)
print(labels.shape)
...
x_train_distant = samples_distant[:4000]
x_test_distant = samples_distant[4000:]
adjacency_train_distant = adjacencys[:4000]
y_train = labels[:4000]
y_test = labels[4000:]
adjacency_test_distant = adjacencys[4000:]

print(x_train_distant.shape,y_train.shape,adjacency_train_distant.shape)
print(x_test_distant.shape,y_test.shape,adjacency_test_distant.shape)


################################################ Model: distant Block - Multi-STGCnet
...
callbacks_list = [
    keras.callbacks.ModelCheckpoint(filepath='distant.h5',
                                    monitor='val_loss',
                                    save_best_only=True,)
                 ]
...
################################################ Model training
epochs = 1000
H = model.fit([x_train_distant,adjacency_train_distant], y_train,callbacks=callbacks_list,batch_size=interval_batch,epochs=epochs,validation_split=0.2)
...
predict_best = model_best.predict([x_test_distant,adjacency_test_distant])
